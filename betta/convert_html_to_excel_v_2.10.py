...
def get_tables(html_path, format_table):
    ...
    if format_table == "file":
        ...
        sys.exit(1)
    tables = find_all_possible_tables(soup)
    return tables
...
def data_acquisition():
    ...
    html_path = 'https://2019.ecology-gosdoklad.ru/report/7/'  # sys.argv[2]
...
def find_all_possible_tables(soup):
    try:


        tables = set()  # используем множество для избежания дубликатов

        # 1. Поиск по стандартным тегам table
        tables.update(soup.find_all('table', recursive=True))

        # 2. Поиск по классам содержащим слово table
        tables.update(soup.find_all(class_=re.compile('.*table.*', re.I)))

        # 3. Поиск по ID содержащим слово table
        tables.update(soup.find_all(id=re.compile('.*table.*', re.I)))

        # 4. Поиск div'ов с табличной структурой
        tables.update(soup.find_all('div', {'role': 'table'}))

        # 5. Поиск элементов с табличной структурой
        all_elements = soup.find_all()
        for element in all_elements:
            # Проверяем наличие табличных элементов
            if (element.find_all('tr') or
                    element.find_all('td') or
                    element.find_all('th')):
                tables.add(element)

            # Проверяем атрибуты на наличие слова table
            if element.attrs:
                for attr, value in element.attrs.items():
                    if isinstance(value, str) and 'table' in value.lower():
                        tables.add(element)
                    elif isinstance(value, list) and any('table' in v.lower() for v in value):
                        tables.add(element)

        # 6. Поиск в iframe (если есть)
        iframes = soup.find_all(['frame', 'iframe'])
        for iframe in iframes:
            ...
            if src:
                try:
                    ...
                    tables.update(find_all_possible_tables(src))
                except:
                    continue

        # 7. Поиск через CSS-селекторы
        tables.update(soup.select('table, .table, [class*="table"], [id*="table"]'))

        # Преобразуем результат в список и фильтруем None значения
        tables = [t for t in list(tables) if t is not None]
