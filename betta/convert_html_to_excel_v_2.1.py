...
def flip_table_vertical(rows_data):
    ...
    for i in range(len(rows_data)):
        ...
        for j in range(len(rows_data[i])):
            flip_data[i] += [rows_data[i][j]]
    for i in range(len(rows_data)):
        flip_data[i] = flip_data[i][::-1]
    return flip_data
    """
...
def chek(table):
    ...
    table_span = get_tables_with_colspan_and_rowspan(table)

    up = []
    for i in range(len(table_span)):
        up += [[]]
        for j in range(len(table_span[i])):
            cell = table_span[i][j]
            value = cell['value']
            rowspan = cell['rowspan']
            colspan = cell['colspan']
    ...
    left = transpose_table_structure(table_span)
    if vertical_chek(left):
        print("left")
        return True
    
    right = transpose_table_structure( flip_table_vertical(table_span) )
    if vertical_chek(right):
        print("right")
        return True

    bottom = transpose_table_structure( flip_table_vertical( transpose_table_structure(table_span) ) )
    if vertical_chek(bottom):
        print("bottom")
        return True
    print("Nan")
    return False
...
def get_genuine_tables(tables):
    ...
    genuine_tables=[]
    for table in tables:
        if not(table.find("table")) and chek(table):
